...
| 중심 갱신 후 전체 소속도 재계산 | 중심 업데이트 `10`회 |
...
### 9.2 전체 소속도 재계산과 재클러스터링

- 중심 업데이트가 10회 누적되면 모든 누적 문서를 현재 중심에 다시 배정한다.
- 이때 거리 임계값, assignment, 트리의 개수와 계층 가중 XB를 갱신한다.
- 현재 XB가 초기 기준 XB보다 `5%` 이상 악화되면 누적 데이터 전체를
  재클러스터링한다.
- 신규 배치의 자연 노이즈 비율이 기본 `5%`를 초과하면 주기와 관계없이
  즉시 전체 재클러스터링한다.
...
`emergency_recluster`, `xb_relative_degradation`, `new_noise_ratio` 등이
기록된다.

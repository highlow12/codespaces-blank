...
> 이 리뷰의 `current`는 커밋 `0c4613a`를 뜻하며 현재 작업 트리를 뜻하지 않는다.
> 당시 최적화의 재현 기록은 유지하고, 최신 기본값과 fuzzifier 검증은
> [`CURRENT_ALGORITHM.md`](CURRENT_ALGORITHM.md) 및 루트 `README.md`를 참조한다.

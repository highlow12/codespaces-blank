...
모든 기존 평면 클러스터링 파이프라인을 포팅할 필요는 없다. PCA + FCM 계층형 파이프라인과 그 시각화만 구현한다.
...
- 랜덤 초기화에는 `Math.random()`을 사용하지 말고 seeded PRNG를 사용한다.
...
중심 벡터의 길이가 `epsilon`보다 작으면 입력 샘플 하나를 seeded random으로 골라 중심으로 사용한 후 정규화한다.

### 6.3 거리와 membership 업데이트
...
`maxIter`에 도달하면 마지막 membership를 사용한다. 최종 hard label은 각 행에서 membership가 가장 큰 클러스터의 인덱스다.
...
각 FCM 후보에 대해 hard label을 만든 뒤 노이즈를 판정한다.
...
`kSelection="knee"`일 때는 후보의 FCM objective 곡선을 사용한다.

1. objective를 후보 순서에 따라 `[0, 1]` 범위로 정규화한다.
2. 첫 후보와 마지막 후보를 잇는 직선을 만든다.
3. 각 후보와 직선 사이의 수직 거리를 계산한다.
4. 거리가 가장 큰 후보를 knee로 선택한다.

실루엣이 유효한 후보만 knee 후보로 사용한다.
...
recurse(indices, node, depth):
  ...
  각 생존 클러스터에 대해:
    현재 level에 0-based cluster id 기록
    child node 생성
    child에 대해 recurse()
```
...
6. noise를 하나의 일반 클러스터로 취급하지 않는다.
7. UMAP 좌표의 절대적인 방향/축 크기는 의미가 없다. seed와 파라미터가 같은 경우의 상대적 구조만 비교한다.
8. 시각화 색상은 클러스터링 결과를 바꾸지 않는다.
9. 모든 평면 파이프라인을 JS로 포팅하지 않는다. 이 문서의 범위는 계층형 PCA + 구면 FCM이다.
...
ID 기준으로 임베딩과 assignment를 one-to-one join한다. 행 순서가 같다는 가정만으로 합치지 않는다.
...
- 최상위 클러스터마다 서로 다른 hue를 사용한다.
- 같은 최상위 클러스터의 하위 클러스터는 동일한 hue의 밝기 차이로 표시한다.
- 하위 라벨의 정렬 순서는 문자열 정렬이 아니라 숫자 자연 정렬을 사용한다.
- noise, `*-noise`는 모두 회색 `#9aa0a6`로 표시한다.
- 최상위 클러스터 수가 10개 이하이면 구분 가능한 categorical palette를 사용한다.
...
- assignment와 좌표의 ID를 기준으로 join한다.
- 범례에는 `cluster label (count)` 형식으로 각 라벨의 샘플 수를 표시한다.
- 축 이름은 `UMAP-1`, `UMAP-2`로 표시한다.
- 제목에는 `PCA-32 + UMAP`, 색상 기준, hierarchical 여부를 표시한다.
- 브라우저에서는 hover 시 `id`, 원본 메타데이터, 표시 라벨, `cluster_path`를 보여준다.
...
- 실루엣 계산은 기본적으로 정확한 방식으로 구현하되, 대형 데이터셋을 위해 `silhouetteSampleSize` 또는 blockwise 계산 옵션을 제공한다.
- UMAP 비교 모드는 의도적으로 UMAP을 네 번 실행한다. 일반 모드는 한 번만 실행한다.
- 메모리상 원본 임베딩, PCA64 행렬, PCA32 행렬을 동시에 복제하지 않도록 typed array와 view를 사용한다.
- 긴 작업은 진행률을 출력한다.
...
- L2 정규화 후 각 행의 norm이 1에 가깝다.
- FCM membership 각 행의 합이 1에 가깝다.
- FCM 중심의 norm이 1에 가깝다.
- 동일한 seed로 실행하면 동일한 결과가 나온다.
- 모든 생존 자식 클러스터의 크기가 `minChildSize` 이상이다.
- noise 샘플은 하위 재귀에 다시 들어가지 않는다.
- `level_N_cluster=-1` 이후의 계층 값도 모두 `-1`이다.
- tree의 child path는 parent path의 접두사다.
- assignment의 `cluster_path`와 tree의 leaf path가 일치한다.
- ID가 섞여 있어도 row order가 아니라 ID 기준으로 시각화 데이터가 매칭된다.
...
JS의 PRNG, PCA SVD, UMAP 구현이 Python과 다르면 샘플별 label 번호가 달라질 수 있다. 이 경우 raw label 배열을 그대로 비교하지 말고 다음을 비교한다.

- 계층 깊이
- leaf 수
- noise 비율
- silhouette/NMI/ARI 범위
- 같은 최상위 주제의 분리 구조
...
- [ ] JSON 배열과 `records` checkpoint를 모두 읽는다.
- [ ] PCA64 + 구면 FCM + 재귀 계층화를 실행한다.
- [ ] silhouette/knee 기반 가변 `k`를 지원한다.
- [ ] membership/거리/최소 크기 기반 noise 처리를 지원한다.
- [ ] 최대 4계층과 조기 중단을 지원한다.
- [ ] assignments CSV/JSON과 tree JSON을 출력한다.
- [ ] `3-2-4` 형식의 표시 라벨을 생성한다.
- [ ] 최상위 hue 분리와 하위 hue 계열 색상을 구현한다.
- [ ] PCA32 + UMAP 시각화를 구현한다.
- [ ] balanced 기본 UMAP 설정과 네 가지 비교 프리셋을 지원한다.

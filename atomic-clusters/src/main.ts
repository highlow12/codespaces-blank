...
export default class AtomicClustersPlugin extends Plugin {
  ...
  async onunload(): Promise<void> { await this.worker?.terminate(); this.worker = null; this.sqliteStore?.close(); this.sqliteStore = null; disposeLocalOrtAssets(); }
  ...
  private async buildClusters(): Promise<void> {
    ...
    try {
      ...
      await sqlite.upsertNotes(notes);

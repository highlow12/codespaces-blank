...
export class AtomicClustersSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl).setName("Gemini model").addText((text) => text.setValue(this.settings.geminiModel).onChange(async (value) => { this.settings.geminiModel = value.trim() || "gemini-embedding-2"; await this.save(); this.onRefreshSettingsChange?.(); }));
    ...
    if (this.settings.embeddingProvider === "local") {
      ...
      const refresh = async (verify = false) => { if (verify) { progress.value = 0; statusEl.setText("Verifying model integrity…"); } try { const status = verify ? await this.localModels.verifyStatus() : await this.localModels.status(); statusEl.setText(status === "installed" ? (verify ? "Installed and integrity verified" : "Installed (manifest ready; use Check model to verify integrity)") : status === "corrupt" ? "Corrupt — delete and download again" : "Missing — download to enable offline inference"); progress.value = status === "installed" ? 1 : 0; } catch (error) { statusEl.setText(`${verify ? "Integrity check failed" : "Status check failed"}: ${safeUiError(error)}`); progress.value = 0; } };
      let checkButton: { setDisabled(disabled: boolean): unknown };
      modelSetting.addButton((button) => { checkButton = button; return button.setButtonText("Check model").onClick(async () => { setBusy(true); try { await refresh(true); } finally { setBusy(false); } }); }); controls.push(checkButton!);

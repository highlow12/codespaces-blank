...
export class ClusterExplorerView extends ItemView {
  ...
  private renderVisualization(): void { this.renderGlobalVisualization(); }
  ...
  private renderLegacyVisualization(): void {
    ...
    const draw = (viewport?: VisualizationViewport): boolean => {
      ...
      const labelPlacements = layoutVisualizationClusterLabels(frontier, points, result.titles, palette.nodeColors, width, height, { measureText: (text) => typeof context.measureText === "function" ? context.measureText(text).width : text.length * 7 });
  ...
  private renderGlobalVisualization(): void {
    ...
    const frontier = visualizationGlobalDepthFrontier(root, this.visualizationDepth, this.visualizationSelectedNodeId, memberships, ordering, result.hierarchyPlacements); const frame = this.contentEl.createDiv({ cls: "atomic-clusters-umap" }); const plot = frame.createDiv({ cls: "atomic-clusters-umap-plot" }); const navigation = plot.createDiv({ cls: "atomic-clusters-umap-navigation" });
    ...
    back.addEventListener("click", () => { if (this.visualizationDepth <= 0) this.visualizationSelectedNodeId = null; else { this.visualizationDepth--; const path = this.visualizationSelectedNodeId ? visualizationPath(root, this.visualizationSelectedNodeId) : []; this.visualizationSelectedNodeId = path.length > 2 ? path[path.length - 2].id : null; } this.visualizationNodeId = this.visualizationSelectedNodeId || "root"; this.visualizationCameraState = null; this.render(); });
    const crumbs = navigation.createDiv({ cls: "atomic-clusters-breadcrumb" }); selectedPath.forEach((node, index) => { if (index) crumbs.createSpan({ text: " / " }); const title = node.sourceId === null ? "All notes" : result.titles?.[String(node.sourceId)] || `Cluster ${node.sourceId}`; const crumb = crumbs.createEl("button", { text: title, attr: { type: "button", "aria-current": index === selectedPath.length - 1 ? "page" : "false" } }); crumb.addEventListener("click", () => { if (node.id === "root") { this.visualizationDepth = 0; this.visualizationSelectedNodeId = null; } else this.visualizationSelectedNodeId = node.id; this.visualizationCameraState = null; this.render(); }); });
    ...
    const baseSigma = visualizationBaseBandwidth(coordinates); const p95 = visualizationP95RowSum(memberships); const pointHitRadius = 14; const pointHitButtons = new Map<number, HTMLButtonElement>(); let camera: VisualizationCamera; let points: [number, number][] = []; let width = 0; let height = 0; let cachedKey = ""; let cachedBitmap: HTMLCanvasElement | null = null; let cachedClouds: VisualizationSplat[][] = []; let rasterCameraState: VisualizationCameraState | null = null; let densityTimer: number | null = null; let suppressClick = false;
    const rectSize = (): [number, number] | null => { const rect = canvas.getBoundingClientRect(); const w = rect.width || plot.clientWidth; const h = rect.height || plot.clientHeight; return w > 0 && h > 0 ? [Math.max(1, Math.round(w)), Math.max(1, Math.round(h))] : null; };
    const bitmapFor = (splats: readonly VisualizationSplat[], opacity: number): HTMLCanvasElement | null => { if (typeof document === "undefined") return null; const field = accumulateVisualizationDensity(splats, width, height); const bitmap = document.createElement("canvas"); bitmap.width = width; bitmap.height = height; const bitmapContext = bitmap.getContext("2d"); if (!bitmapContext) return null; const image = bitmapContext.createImageData(width, height); for (let i = 0; i < field.density.length; i++) { const density = field.density[i]; image.data[i * 4] = density > 0 ? Math.round(field.red[i] / density) : 0; image.data[i * 4 + 1] = density > 0 ? Math.round(field.green[i] / density) : 0; image.data[i * 4 + 2] = density > 0 ? Math.round(field.blue[i] / density) : 0; image.data[i * 4 + 3] = Math.round(visualizationDensityAlpha(density) * opacity * 255); } bitmapContext.putImageData(image, 0, 0); return bitmap; };
    const terminalPath = (index: number): string[] => visualizationNoteTerminalPath(root, index, labels, result.hierarchyPlacements); const pointActive = (index: number): boolean => !this.visualizationSelectedNodeId || terminalPath(index).includes(this.visualizationSelectedNodeId);
    const activatePoint = (index: number): void => { const path = terminalPath(index); const selected = this.visualizationSelectedNodeId; if (selected && path[path.length - 1] === selected) { void this.app.workspace.openLinkText(result.ids[index], "", false); return; } if (path.length === 1) { if (this.visualizationDepth === 0 && !selected) void this.app.workspace.openLinkText(result.ids[index], "", false); else { this.visualizationSelectedNodeId = null; this.visualizationDepth = 0; this.render(); } return; } const entry = frontier.find((item) => item.pointIndices.includes(index)); if (entry) { this.visualizationSelectedNodeId = entry.node.id; if (entry.node.children.length) this.visualizationDepth++; this.visualizationNodeId = entry.node.id; this.visualizationCameraState = null; this.render(); return; } const next = path.find((id) => id !== "root"); if (next) { this.visualizationSelectedNodeId = next; this.visualizationDepth++; this.visualizationNodeId = next; this.visualizationCameraState = null; this.render(); } };
    const activateCluster = (node: VisualizationNode): void => { this.visualizationSelectedNodeId = node.id; if (node.children.length) this.visualizationDepth++; this.visualizationNodeId = node.id; this.visualizationCameraState = null; this.render(); };
    const draw = (reraster = true): boolean => { const size = rectSize(); if (!size) return false; [width, height] = size; const dpr = Math.max(1, typeof window === "undefined" ? 1 : window.devicePixelRatio || 1); canvas.width = width * dpr; canvas.height = height * dpr; const context = canvas.getContext("2d"); if (!context) return false; context.setTransform(dpr, 0, 0, dpr, 0, 0); context.clearRect(0, 0, width, height); if (!this.visualizationCameraState || this.visualizationCameraState.width !== width || this.visualizationCameraState.height !== height) { const fitted = visualizationFitCameraState(coordinates, width, height); const old = this.visualizationCameraState; this.visualizationCameraState = old ? { ...fitted, centerX: old.centerX, centerY: old.centerY, zoom: old.zoom } : fitted; } camera = visualizationCameraFromState(this.visualizationCameraState); points = coordinates.map((point) => visualizationWorldToScreen(camera, point)); this.visualizationPoints = points; const key = `${this.visualizationDepth}|${this.visualizationSelectedNodeId || ""}|${camera.scale}|${camera.offsetX}|${camera.offsetY}|${width}x${height}|${this.visualizationKernelScale}`; if (reraster && key !== cachedKey) { cachedKey = key; const active: VisualizationSplat[] = []; const inactive: VisualizationSplat[] = []; cachedClouds = []; for (const entry of frontier) { const value = visualizationCloudColor(entry.node, palette); const color = /^#[0-9a-f]{6}$/i.test(value) ? [parseInt(value.slice(1, 3), 16), parseInt(value.slice(3, 5), 16), parseInt(value.slice(5, 7), 16)] as [number, number, number] : visualizationColorVector([], ordering); const splats: VisualizationSplat[] = []; for (const index of entry.pointIndices) { const point = points[index]; if (!point) continue; splats.push({ x: point[0], y: point[1], sigma: visualizationScaledStageSigma(baseSigma, entry.remainingDepth, !entry.node.children.length, this.visualizationKernelScale) * camera.scale, color, amplitude: visualizationMembershipAmplitude(memberships[index] || [], p95) }); } cachedClouds.push(splats); (entry.active ? active : inactive).push(...splats); } const activeBitmap = bitmapFor(active, 1); const inactiveBitmap = bitmapFor(inactive, .2); cachedBitmap = document.createElement("canvas"); cachedBitmap.width = width; cachedBitmap.height = height; const merged = cachedBitmap.getContext("2d"); if (merged) { if (inactiveBitmap) merged.drawImage(inactiveBitmap, 0, 0); if (activeBitmap) merged.drawImage(activeBitmap, 0, 0); } } if (cachedBitmap) context.drawImage(cachedBitmap, 0, 0); const style = typeof getComputedStyle === "function" ? getComputedStyle(frame) : null; const background = style?.getPropertyValue("--background-primary").trim() || "transparent"; for (let index = 0; index < points.length; index++) { const point = points[index]; const placement = result.hierarchyPlacements?.[index]; context.beginPath(); context.fillStyle = labels[index] === -1 || placement?.kind === "residual" ? VISUALIZATION_NOISE_COLOR : blendVisualizationColor(memberships[index] || [], ordering, palette.leafColors); context.globalAlpha = this.hoveredVisualizationPoint === index ? 1 : pointActive(index) ? 1 : .2; context.arc(point[0], point[1], this.hoveredVisualizationPoint === index ? 6 : 4, 0, Math.PI * 2); context.fill(); context.globalAlpha = 1; context.strokeStyle = background; context.stroke(); } const labelsOnCanvas = layoutVisualizationClusterLabels(frontier, points, result.titles, palette.nodeColors, width, height); if (typeof context.fillText === "function") for (const label of labelsOnCanvas) { const entry = frontier.find((item) => item.node.id === label.id); context.save(); context.globalAlpha = entry?.active ? 1 : .2; context.fillStyle = entry ? palette.nodeColors.get(entry.node.id) || VISUALIZATION_NOISE_COLOR : VISUALIZATION_NOISE_COLOR; context.fillRect(label.x, label.y, label.width, label.height); context.fillStyle = label.contrast.foreground; context.font = "600 12px system-ui, sans-serif"; context.textAlign = "center"; context.textBaseline = "middle"; context.fillText(label.text, label.x + label.width / 2, label.y + label.height / 2); context.restore(); } this.visualizationRenderedPointIndices = points.map((_point, index) => index); this.visualizationSpatialIndex = buildVisualizationPointSpatialIndex(points, pointHitRadius * 2); const pool = this.visualizationSpatialIndex.queryRect(-pointHitRadius, -pointHitRadius, width + pointHitRadius, height + pointHitRadius).slice(0, 96); const poolSet = new Set(pool); for (const [index, hit] of pointHitButtons) if (!poolSet.has(index)) { hit.remove(); pointHitButtons.delete(index); } this.visualizationHitElements = pool.map((index) => { let hit = pointHitButtons.get(index); if (!hit) { hit = hitLayer.createEl("button", { cls: "atomic-clusters-umap-point-hit", attr: { type: "button", "data-point-index": String(index), "aria-label": result.ids[index] } }); hit.addEventListener("mouseenter", (event) => this.setHoveredVisualizationPoint(index, event, hit!, () => draw(false))); hit.addEventListener("click", (event) => { event.preventDefault(); activatePoint(index); }); pointHitButtons.set(index, hit); } hit.style.left = `${points[index][0]}px`; hit.style.top = `${points[index][1]}px`; return hit; }); return true; };
    const updateLayerTransform = (): void => { if (!rasterCameraState || !this.visualizationCameraState) return; const source = visualizationCameraFromState(rasterCameraState); const target = visualizationCameraFromState(this.visualizationCameraState); const ratio = target.scale / source.scale; visualLayer.style.transformOrigin = "0 0"; visualLayer.style.transform = `translate(${target.offsetX - ratio * source.offsetX}px, ${target.offsetY - ratio * source.offsetY}px) scale(${ratio})`; };
    ...
    let dragging = false; let moved = false; let startX = 0; let startY = 0; let startState: VisualizationCameraState | null = null; const onPointerDown = (event: PointerEvent): void => { if (event.button !== 0) return; dragging = true; moved = false; startX = event.clientX; startY = event.clientY; startState = this.visualizationCameraState ? { ...this.visualizationCameraState } : null; const target = event.currentTarget as (HTMLElement & { setPointerCapture?: (pointerId: number) => void }) | null; target?.setPointerCapture?.(event.pointerId); }; const onPointerMove = (event: PointerEvent): void => { if (!dragging || !startState) return; const dx = event.clientX - startX, dy = event.clientY - startY; if (!moved && Math.hypot(dx, dy) < 4) return; moved = true; suppressClick = true; this.visualizationCameraState = panVisualizationCamera(startState, dx, dy); updateLayerTransform(); }; const onPointerUp = (): void => { if (!dragging) return; dragging = false; if (moved) scheduleDensity(); startState = null; }; const onWheel = (event: WheelEvent): void => { event.preventDefault(); const rect = canvas.getBoundingClientRect(); this.visualizationCameraState = zoomVisualizationCameraAt(this.visualizationCameraState || visualizationFitCameraState(coordinates, width || 1, height || 1), event.clientX - rect.left, event.clientY - rect.top, Math.exp(-event.deltaY * .001)); updateLayerTransform(); scheduleDensity(); };
    const onClick = (event: MouseEvent): void => { if (suppressClick) { suppressClick = false; return; } if (event.target !== canvas) return; const rect = canvas.getBoundingClientRect(); const index = this.visualizationSpatialIndex?.queryNearest(event.clientX - rect.left, event.clientY - rect.top, pointHitRadius) ?? null; if (index !== null) { activatePoint(index); return; } const picked = pickVisualizationCloud(cachedClouds, event.clientX - rect.left, event.clientY - rect.top); if (picked !== null && frontier[picked]) activateCluster(frontier[picked].node); };
    const onMouseMove = (event: MouseEvent): void => { if (dragging) return; const rect = canvas.getBoundingClientRect(); const index = this.visualizationSpatialIndex?.queryNearest(event.clientX - rect.left, event.clientY - rect.top, pointHitRadius) ?? null; this.setHoveredVisualizationPoint(index, event, canvas, () => draw(false)); }; const onMouseLeave = (): void => this.setHoveredVisualizationPoint(null, null, null, () => draw(false));
    visualLayer.addEventListener("pointerdown", onPointerDown); visualLayer.addEventListener("pointermove", onPointerMove); visualLayer.addEventListener("pointerup", onPointerUp); visualLayer.addEventListener("pointercancel", onPointerUp); visualLayer.addEventListener("wheel", onWheel, { passive: false }); visualLayer.addEventListener("click", onClick); canvas.addEventListener("mousemove", onMouseMove); canvas.addEventListener("mouseleave", onMouseLeave); this.visualizationCleanup = () => { if (densityTimer !== null) globalThis.clearTimeout(densityTimer); visualLayer.removeEventListener("pointerdown", onPointerDown); visualLayer.removeEventListener("pointermove", onPointerMove); visualLayer.removeEventListener("pointerup", onPointerUp); visualLayer.removeEventListener("pointercancel", onPointerUp); visualLayer.removeEventListener("wheel", onWheel); visualLayer.removeEventListener("click", onClick); canvas.removeEventListener("mousemove", onMouseMove); canvas.removeEventListener("mouseleave", onMouseLeave); this.visualizationHitElements = []; this.visualizationSpatialIndex = null; }; draw(true); if (typeof ResizeObserver === "function") { this.visualizationResizeObserver = new ResizeObserver(() => { cachedKey = ""; cachedBitmap = null; draw(true); }); this.visualizationResizeObserver.observe(plot); }

...
export class ClusterExplorerView extends ItemView {
  ...
  private renderFocusControls(parent: HTMLElement): void {
    ...
    const title = node?.sourceId === null ? "All notes" : this.result.titles?.[String(node?.sourceId)] || `Cluster ${node?.sourceId ?? this.clusterKeyForNodeId(this.focusNodeId)}`;
  ...
  private renderGlobalVisualizationReliable(): void {
    ...
    const terminalPath = (index: number): string[] => visualizationNoteTerminalPath(root, index, labels, result.hierarchyPlacements); const pointActive = (index: number): boolean => (!this.visualizationSelectedNodeId || terminalPath(index).includes(this.visualizationSelectedNodeId)) && (!searchActive || matchedNotes.has(result.ids[index]));
    const cameraFitOptions = (context: CanvasRenderingContext2D) => ({ pointRadius, hoverPointRadius: hoverRadius, labelBoxes: measureVisualizationClusterLabelBoxes(frontier, result.titles, (text) => measureTextFallback(context, text)), labelMargin: 8 });
    ...
    const draw = (reraster = true, viewport?: VisualizationViewport): boolean => {
      ...
      const labelMargin = Math.min(Math.max(2, this.visualizationCameraState!.padding), Math.min(width, height) / 2); const labelPlacements = layoutVisualizationClusterLabels(frontier, points, result.titles, palette.nodeColors, width, height, { margin: labelMargin, measureText: (text) => measureTextFallback(context, text) });

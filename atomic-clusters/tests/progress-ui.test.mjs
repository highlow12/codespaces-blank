...
test("installed plugin exposes persistent progress for model and clustering flows", async () => {
  ...
  assert.match(main, /preflightRendererClusteringMemory/);
  assert.match(main, /phase: "memory preflight"/);
  assert.match(main, /if \(!memoryPreflight\.canProceed\)/);
  assert.ok(main.indexOf("const memoryPreflight") < main.indexOf("const worker = await this.getWorker()"));
